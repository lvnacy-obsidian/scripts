...
const logCtx = {
	id: 'baseModal',
	name: 'Base Modal',
	path: 'library-scripts/service/baseModal.js',
};
...
async function loadModalStyles(app) {
	try {
		...
		if (!document.getElementById('modal-styles')) {
			...
			Log.info(logCtx, '✅ Modal CSS styles loaded successfully');
		} else {
			Log.info(logCtx, '📝 Modal CSS styles already loaded');
		}
	} catch (error) {
		Log.error(logCtx, '❌ Failed to load modal styles:', error);
	}
}
...
function setModalBackground(app, imagePath) {
	...
	try {
		...
		Log.error(logCtx, 'Error getting resource path:', error);
		// Method 3: Final fallback - simple vault resource URL
		resourcePath = `app://obsidian.md/${imagePath}`;
	}
	
	Log.info(logCtx, 'Setting background image path:', resourcePath);
...
export class BaseModal extends Modal {
	...
	async openAndGetValue(resolve, reject) {
		this.resolve = await resolve;
		this.reject = await reject;
		this.open();
	}
